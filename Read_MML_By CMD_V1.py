...
def get_folder_name(file_path):  #获取文件夹名
...
def export(path):
    for file in open_file():
        ...
        os.chdir(sub_path)
        excelWriter = pd.ExcelWriter('MML_Output.xls')
        for mml,val in read_command(file).items():
            df=pd.DataFrame(val)
            try:
                if df.shape[0]<65534:
                    df.to_excel(excel_writer=excelWriter,sheet_name=mml,header=True,index=True,merge_cells=False)
                    print(mml)
                else:
                    i=int(df.shape[0]/65534)+1
                    for i in list(range(i)):
                        (df[(65534 * i):(65534 * (i + 1))].to_excel(excel_writer=excelWriter, sheet_name=mml + str(i),
                                                                   header=True, index=True, merge_cells=False))
                        print(mml+str(i))
            except:
                continue
