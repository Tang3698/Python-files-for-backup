...
os.chdir(r'D:\Thailand-AIS\KPI\UMTS')

def open_file():
    file_names = tkinter.filedialog.askopenfilenames(initialdir=r'D:\Thailand-AIS\KPI\UMTS')
    if file_names != '':
        string_filename=""
...
error_log='execute log_umts_ '+ str(time.time())+'.csv'
...
for file in open_file():
    ...
    for line in prs_kpi:                  #for line in  f.readlines()[999:len(f.readlines())-1]: 从某一行开始读
        if '2018' in line:
            ...
            sql_line="INSERT INTO `daily_cell_sts_5.2` VALUES " + '('+new_line+')'+';'
            try:
